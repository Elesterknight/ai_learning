...
class SiglipModel(PreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask, pixel_values):
        ...
        vision_features = vision_features / vision_features.norm(p=2, dim=-1, keepdim=True) # l2標準化
        text_features = text_features / text_features.norm(p=2, dim=-1, keepdim=True) # l2標準化
        
        logits_per_text = torch.matmul(text_features, vision_features.t()) * self.t.exp() + self.b
        logits_per_image = logits_per_text.t()
        
        b = logits_per_text.shape[0]
        eye = torch.eye(b, device=logits_per_text.device) # 生成單位矩陣
        labels = 2*eye - torch.ones_like(logits_per_text, device=logits_per_text.device) # 對角線全為1，非對角線為-1，即成對的圖文標籤為1，非成對的為-1
