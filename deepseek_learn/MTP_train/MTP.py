        ...
        with torch.no_grad():
            ...
            while seq.size(1) < max_length:
                ...
                if validation_tokens.shape[1] > 0:
                    ...
                    # 保留機率值大於閾值的token, 接受這部分token,否則捨棄（捨棄某個token時，後面的token都要捨棄）
                    # 接受token的遮罩
                    accept_mask = (accept_probs > 1e-6)
                    print(f'接受遮罩：{accept_mask}')
                    print(f'拒絕遮罩：{~accept_mask}')
                    # 取得被拒絕（捨棄）token對應的索引
                    reject_token_index = (~accept_mask).nonzero(as_tuple=True)[1]
                    print(f'拒絕token的索引：{reject_token_index}')
                    # 如果有需要捨棄的token
                    if reject_token_index.shape[0] > 0:
                        
                        # 第一個token預設接受，後續接受數量由第一個被拒絕的mtp token決定
                        accept_num += reject_token_index[0].item()
                    
                    else:
                        # 如果沒有需要捨棄的token，則全部接受
