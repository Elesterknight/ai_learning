...
class MLA(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.dim = dim # 隱藏層維度
        self.n_heads = n_heads  #總頭數
        self.q_lora_rank = q_lora_rank # q低秩壓縮到的維度
        self.kv_lora_rank = kv_lora_rank # kv低秩壓縮到的維度
        self.qk_nope_head_dim = qk_nope_head_dim
        self.qk_rope_head_dim = qk_rope_head_dim
        self.qk_head_dim = qk_nope_head_dim + qk_rope_head_dim # qk的總維度，不帶旋轉位置編碼的維度加上帶旋轉位置編碼的維度
        self.v_head_dim = v_head_dim # value的維度，等於不帶旋轉位置編碼的k維度
        self.mode = mode
        self.max_seq_len = max_seq_len
        self.max_batch_size = max_batch_size
        
        
        self.wq_a = nn.Linear(self.dim, self.q_lora_rank) # q的降維矩陣
        self.q_norm = RMSNorm(self.q_lora_rank)
        self.wq_b = nn.Linear(self.q_lora_rank, self.n_heads * self.qk_head_dim) # q的升維矩陣
        # 4096*128+128*4864 = 524,288 + 622592 = 1146880    4096*4864 = 19,922,944
        
        self.wkv_a = nn.Linear(self.dim, self.kv_lora_rank + self.qk_rope_head_dim) # kv的降維矩陣
        # nn.Linear(self.dim, self.kv_lora_rank)
        # nn.Linear(self.dim, self.qk_rope_head_dim)
        self.kv_norm = RMSNorm(self.kv_lora_rank)
        self.wkv_b = nn.Linear(self.kv_lora_rank, self.n_heads * (self.qk_nope_head_dim + self.v_head_dim)) # kv的升維矩陣
        
        self.wo = nn.Linear(self.n_heads * self.v_head_dim, self.dim)
        
        self.rotary_emb = RotaryEmbedding(self.qk_rope_head_dim) # 旋轉旋轉位置編碼
    ...
    def forward(self, x, start_pos: int, mask=None):
        ...
        if self.mode == 'naive':
            ...
            # q*k(T) = x*wq*(c*wkv_b[:, :self.qk_nope_head_dim])(T) = x*wq*wkv_b[:, :self.qk_nope_head_dim](T)*c(T)    c為壓縮後的kv
            # wq*wkv_b[:, :self.qk_nope_head_dim](T)作為q的投影矩陣  c可以替代原先的k，這樣就可以直接使用壓縮後的kv計算注意力了，kv_caceh時也只需存儲壓縮後的kv
            kv = self.kv_norm(kv)
