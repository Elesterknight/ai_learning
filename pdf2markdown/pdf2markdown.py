...
DEFAULT_PROMPT = """使用markdown語法，將圖片中識別到的文字轉換為markdown格式輸出。你必須做到：
1. 輸出和使用識別到的圖片的相同的語言，例如，識別到英語的字段，輸出的內容必須是英語。
2. 不要解釋和輸出無關的文字，直接輸出圖片中的內容。例如，嚴禁輸出 “以下是我根據圖片內容生成的markdown文本：”這樣的例子，而是應該直接輸出markdown。
3. 內容不要包含在```markdown ```中、段落公式使用 $$ $$ 的形式、行內公式使用 $ $ 的形式、忽略掉長直線、忽略掉頁碼。
再次強調，不要解釋和輸出無關的文字，直接輸出圖片中的內容。
"""
DEFAULT_RECT_PROMPT = """圖片中用帶顏色的矩形框和名稱(%s)標註出了一些區域。如果區域是表格或者圖片，使用 ![]() 的形式插入到輸出內容中，否則直接輸出文字內容。
"""
DEFAULT_ROLE_PROMPT = """你是一個PDF文檔解析器，使用markdown和latex語法輸出圖片的內容。
"""
