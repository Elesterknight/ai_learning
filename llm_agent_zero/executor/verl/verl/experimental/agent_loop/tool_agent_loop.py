...
class PythonAgentLoop(AgentLoopBase):
    def __init__(self, config, server_manager, tokenizer):
        super().__init__(config, server_manager, tokenizer)
    ...
    def init_class(cls, config, tokenizer):
        if cls._class_initialized:
            return
        cls._class_initialized = True
        print("Performing class-level PythonAgentLoop initialization")
        ...
        cls.max_tool_response_length = config.actor_rollout_ref.rollout.multi_turn.max_tool_response_length
        cls.tool_response_truncate_side = config.actor_rollout_ref.rollout.multi_turn.tool_response_truncate_side
        cls.sandbox_url = config.reward_model.sandbox_fusion.url
    ...
    async def run(self, messages: List[Dict[str, Any]], sampling_params: Dict[str, Any]) -> AgentLoopOutput:
        ...
        while True:
            ...
            code_block_end_tag = "```"
            start_index = text.find(code_block_start_tag)
            if start_index != -1:
                end_index = text.find(code_block_end_tag, start_index + len(code_block_start_tag))
                if end_index != -1:
                    text = text[:end_index + len(code_block_end_tag)]
            
      
            # response_ids = self.tokenizer.encode(text)
            response_ids = await self.loop.run_in_executor(None, self.tokenizer.encode, text)
            ...
            exec_result = self.code_exec(code_to_run)
            
            if len(exec_result) > self.max_tool_response_length:
                if self.tool_response_truncate_side == "left":
                    exec_result = exec_result[: self.max_tool_response_length] + "...(truncated)"
                elif self.tool_response_truncate_side == "right":
                    exec_result = "(truncated)..." + exec_result[-self.max_tool_response_length :]
                else:
                    length = self.max_tool_response_length // 2
                    exec_result = exec_result[:length] + "...(truncated)..." + exec_result[-length:]
                
            
            tool_response = [{"role": "user", "content": f"Code execution result: {exec_result}"}]
                    
            # tool_response_ids = self.tokenizer.apply_chat_template(messages=tool_response, add_generation_prompt=True, tokenize=True)
    ...
    def code_exec(self, code):
        ...
        try:
            ...
            response = requests.request("POST", self.sandbox_url, headers=headers, data=payload).json()
