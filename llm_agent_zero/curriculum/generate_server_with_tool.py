...
def generate_with_tool_use(question: str, num_candidates: int = 10, max_turns: int = 4):
    ...
    for turn in range(max_turns):
        ...
        for i, response in enumerate(responses):
            ...
            # 取第一個程式碼塊（提示詞裡面寫了生成程式碼塊之後立即停止，但是模型不一定完全遵從指令，有可能生成自己編造執行結果，然後再次生成）
...
def consolidate_and_grade(question, golden_answer, assistant_messages, tool_count):
    ...
    for res in results:
        ...
        for exist_ans in list(answer_counts.keys()):
            ...
            # 如果新答案 res 和以前的某個 exist_ans 完全相等，直接計數+1
            # 或者兩個答案裡都包含 "no "（比如都是否定回答），也當成同一類
    ...
    return {
        'question': question,
        'answer':   majority_ans,
        'score':    score if grade_answer(majority_ans, golden_answer) and score > 0.1 else 0, # 用標準答案再過濾一次（是否真正正確）
