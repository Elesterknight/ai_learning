...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, 
        ...
        if self.flash_attn and s > 1:
            ...
            scores = torch.matmul(q, k.transpose(2, 3)) / math.sqrt(self.head_dim)  # 計算注意力分數
          
            scores = scores + mask  # 應用遮罩
            scores = F.softmax(scores.float(), dim=-1).type_as(q)  # 計算 softmax
            scores = self.attention_dropout(scores)  # 應用注意力 dropout
            output = torch.matmul(scores, v)  # 計算輸出
...
def load_balancing_loss_func(
    ...
    num_experts,
    top_k):
    concatenated_gate_logits = torch.cat([layer_gate for layer_gate in gate_logits], dim=0) # 各個層的gate_logit進行合併[layers X batch_size X sequence_length, num_experts]
...
class Gating(nn.Module):
    ...
    def forward(self, x):
        ...
        logits_topk, indices = logits.topk(self.topk, dim=-1) # 選擇機率最大的兩個專家，回傳兩個專家對每個token的機率
        zeros = torch.full_like(logits, float("-inf")) # 建立一個全為負無窮的矩陣，用於遮蔽其他專家的機率並重新正規化機率最大的兩個專家
        sparse_logits = zeros.scatter(dim=-1, index=indices, src=logits_topk)  # 將選擇的兩個專家的機率按指定索引填充
        sparse_logits = F.softmax(sparse_logits, dim=-1) # 得到一個稀疏矩陣，選擇的兩個專家對每個token的機率和為1
...
class Config(PretrainedConfig):
    ...
    def __init__(self,
        ...
        self.num_attention_heads = num_attention_heads # softmax注意力q頭數
        self.num_key_value_heads = num_key_value_heads # softmax注意力k,v頭數
        
        self.linear_num_value_heads = linear_num_value_heads # 線性注意力v的頭數
        self.linear_num_key_heads = linear_num_key_heads # 線性注意力k的頭數
        self.linear_key_head_dim = linear_key_head_dim # 線性注意力k每個頭的維度
        self.linear_value_head_dim = linear_value_head_dim # 線性注意力v每個頭的維度
        
        self.linear_conv_kernel_dim = linear_conv_kernel_dim # 線性注意力卷積核大小
...
if __name__ == '__main__':   
    ...
    config = Config()
    model = LLM(config)
    print(f'模型參數量為：{sum(p.numel() for p in model.parameters() if p.requires_grad)}')
