...
class GRPOArguments:
    ...
    save_steps = 100
    epoch = 3
    num_generations = 4 # 組內樣本數
    max_prompt_length = 256 # 最大輸入長度
    max_generate_length = 256 # 最大輸出長度
    reward_weights : List[float] = None # 獎勵的權重（多個獎勵函數）
    beta = 0.0 # KL散度的係數，為0則忽略KL散度，即不使用參考模型
    clip_eps_high = 0.28
    clip_eps_low = 0.2
    gradient_accumulation_steps = 2 # 梯度累加
    num_iterations = 1 # 採樣一次樣本訓練模型輪數
    batch_size = 1
...
class GRPOTrainer:
    def __init__(self,
        ...
        # 快取已經生成的資料的一個批次的資料，可供模型多次訓練迭代，無需重新生成
    ...
    def compute_loss(self, model, inputs):
        ...
        coef_1 = torch.exp(action_log_probs - old_action_log_probs) # 重要性採樣 shape: [batch_size * num_generations, num_actions]
        coef_2 = torch.clamp(coef_1, 1 - self.args.clip_eps_low, 1 + self.args.clip_eps_high)
        per_token_loss1 = coef_1 * advantages.unsqueeze(1) # 一個序列中每個token的優勢是一樣的
    ...
    def train(self):
        ...
        for _ in range(self.args.epoch):
            ...
            for batch in dataloader:
                ...
                # 如果生成的樣本batch_size小於設定的batch_size，說明生成資料過程中有捨棄資料，需要繼續採樣，湊夠一個完整的batch_size
