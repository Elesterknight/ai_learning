...
# 價值（評論家）模型，用於預測每一步（生成token）的動作產生的收益，使用演員模型進行初始化，並外加一個迴歸頭，輸出shape為：(batch_size, seq_len， 1)
...
# 最後一個時刻的未來優勢和未來收益為0：A(T+1) = 0, V(T+1) = 0,  則A(T) = R(T) - V(T), 得出A(T)
# A(T-1) = R(T-1) + gam*V(T) - V(T-1) + gam*lam*A(T) 知道A(T)可計算A(T-1) 依次類推
...
def generate_experiences(samples_list):
    ...
    for samples in samples_list:
        ...
        with torch.no_grad():
            ...
            r = reward_model(**reward_model_inputs.to(device)).logits # 獎勵模型的輸出，相當於生成最後一個token的獎勵（結果獎勵模型）
            # 計算kl散度
...
if __name__ == "__main__":
    ...
    # 生成一次經驗，訓練的輪數
    max_epochs = 5
    # 一次從提示詞資料集中取多少條資料用於生成經驗
    rollout_batch_size = 8
    # 一次取多少條資料生成經驗（生成經驗需要多個模型推理，對顯示記憶體要求高）
    micro_rollout_batch_size = 2
    # 一個提示詞生成多少個樣本
    n_samples_per_prompt = 2
    # 生成的最大長度，相當於最大動作數，數值越大，模型探索的可能性越多
    max_new_tokens = 50
    # 最大長度
    max_length = 256
    # 實際訓練的batch_size大小，一次取多少條資料用於更新參數
    ...
    prompt_list = [
        '請問1+1等於多少？',
        'PowerShell，如何知道BIOS中的虛擬化是否已禁用',
        '為什麼人們喜歡在水族館裡游泳，而不是在游泳池裡？',
        '你是一位營銷專家。為Instagram reels寫30個帶有營銷技巧的腳本。',
        '你是一位營銷專家。為Instagram reels寫30個帶有營銷技巧的腳本。',
        '你是一位營銷專家。為Instagram reels寫30個帶有營銷技巧的腳本。',
        '為什麼所有的鏡子都是矩形的？',
        '我們在受感染的植物根部可以找到哪一種，臭氧還是金子？'
    ]
