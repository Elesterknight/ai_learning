...
def create_planner_node(state: State):
    logger.info("***正在執行Create Planner node***")
...
def update_planner_node(state: State):
    logger.info("***正在執行Update Planner node***")
    plan = state['plan']
    goal = plan['goal']
    ...
    while True:
        try:
            ...
            messages += [HumanMessage(content=f"json格式錯誤:{e}")]
            
def execute_node(state: State):
    logger.info("***正在執行execute_node***")
    ...
    logger.info(f"當前執行STEP:{current_step}")
    
    ## 此處只是簡單跳轉到report節點，實際應該根據當前STEP的描述進行判斷
    ...
    logger.info(f"當前STEP執行總結:{extract_answer(response['content'])}")
    # 將當前模型總結添加到歷史上下文
