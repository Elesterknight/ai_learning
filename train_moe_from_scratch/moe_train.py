...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, use_kv_cache=False):
        ...
        if self.flash_attn:
            ...
            # q*k轉置，（b, self.num_heads, s, self.head_dim）* (b, self.num_heads, self.head_dim，s) = （b, self.num_heads, s, s）
            ...
            mask = torch.full((1, 1, self.config.max_seq_len, self.config.max_seq_len), float("-inf"))  # 初始化遮罩
            mask = torch.triu(mask, diagonal=1)  # 生成上三角遮罩
            scores = torch.matmul(q, k.transpose(2, 3)) / math.sqrt(self.head_dim)  # 計算注意力分數
            scores = scores + self.mask[:, :, :s, :s]  # 應用遮罩
...
# 編寫自訂設定時需要記住的三個重要事項如下：
# 1、必須繼承自 PretrainedConfig
# 2、PretrainedConfig 的 __init__ 方法必須接受任何 kwargs
# 3、這些 kwargs 需要傳遞給超類的 __init__ 方法。
